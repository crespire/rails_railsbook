class RichContentValidator < ActiveModel::Validator
  ...
  def validate(record)
    ...
    record.errors.add(:attachment_size, 'too large, file must be 2MB or less.') unless record.content.body.attachments.all? { |attachment| attachment.byte_size <= MAX_FILE_SIZE }
  end
end
